...
import { Terminal, Calendar, Building2, ArrowRight, Code2, Users, Search, Plus, Sparkles } from 'lucide-react';
  ...
  return (
    <div className="pt-20 sm:pt-24 pb-16 sm:pb-20 bg-slate-50 min-h-screen">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 sm:pt-12">
        ...
        {/* Category Filter Pills & Host Button */}
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 mb-8">
          ...
          <Link
            to="/partner-with-us"
            className="inline-flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-xs sm:text-sm font-semibold px-4 py-2.5 rounded-xl shadow-sm shrink-0 whitespace-nowrap text-center"
          >
            ...
            >
              <div>
                <div className="relative h-44 sm:h-48 overflow-hidden bg-slate-900">
                  ...
                  <div className="absolute top-3 left-3 bg-blue-600 text-white text-[10px] sm:text-xs font-bold px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full uppercase tracking-wider shadow-sm">
                    {ws.techCategory}
                  </div>
                </div>

                <div className="p-5 sm:p-6 space-y-3">
                  <h3 className="text-lg sm:text-xl font-bold text-slate-900 font-['Outfit'] group-hover:text-blue-600 transition-colors line-clamp-2 leading-snug">
                    {ws.title}
                  </h3>

                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {ws.techTags.map((tag) => (
                      <span key={tag} className="text-[10px] sm:text-[11px] font-medium bg-blue-50 text-blue-700 px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-md border border-blue-100">
                        {tag}
                      </span>
                    ))}
                  </div>
